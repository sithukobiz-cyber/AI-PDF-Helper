...
async def perform_search_input(update, context, keyword):
    ...
    if not rows:
        await message.edit_text(
            ...
            "🔎 နောက်ထပ် keyword တစ်ခုကို တန်းရိုက်ပို့နိုင်ပါတယ်။",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data="cancel_input")],
                [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")],
            ]),
...
async def ask_ai(prompt):
    ...
    try:
        ...
        if "insufficient_quota" in error_text or "credit_balance_exhausted" in error_text:
            return (
                "⚠️ AI Service ကို ယခုအသုံးမပြုနိုင်သေးပါ။\n\n"
                "OpenAI API credit မလုံလောက်တော့တာကြောင့် "
                "AI response မရနိုင်ပါ။\n\n"
                "👤 Admin က OpenAI API billing/credits ကို စစ်ပေးရပါမယ်။"
            )

        if "model_not_found" in error_text:
            return (
                "⚠️ သတ်မှတ်ထားတဲ့ AI model ကို API project က "
                "အသုံးပြုခွင့်မပေးထားပါ။\n\n"
                "OPENAI_MODEL ကို အသုံးပြုခွင့်ရှိတဲ့ model ID နဲ့ ပြောင်းပါ။"
            )
...
async def ai_message_handler(update, context):
    ...
    if input_mode:
        ...
        # Search mode stays active after every search result.
        # This allows: Search → keyword → Result → next keyword → Result
        # without pressing “Search Again”.
        if input_mode == "search":
            await perform_search_input(update, context, text)
            return

        # Other input modes are single-use.
